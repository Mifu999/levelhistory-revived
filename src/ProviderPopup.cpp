...
bool ProviderPopup::init(std::vector<std::shared_ptr<LevelProvider>> providers) {
    ...
    // v2.0.0: restore the last used search inputs across sessions.
    _enterLevelID = Mod::get()->getSavedValue<std::string>("last-level-id", "");
    _enterQuery = Mod::get()->getSavedValue<std::string>("last-query", "");
...
void ProviderPopup::setupProviderBox(CCLayer *providerBox, LevelProvider *provider) {
    ...
    if (info->_pagesMax >= 2) {
        ...
        CCLabelBMFont *l = CCLabelBMFont::create("Page 1", "chatFont.fnt");
...
void ProviderPopup::setupLevelIDPage(CCLayer *providerBox) {
    ...
    in->setCallback([this](const std::string &value) {
        ...
        Mod::get()->setSavedValue<std::string>("last-level-id", value);
    });
...
void ProviderPopup::lambdaOnDownloadLevel(SearchInstance *si, LoadingCircleLayer *existingCircle, ProviderPopup *popup, LevelProvider *prov, GJGameLevel *level) {
    if (!popup->isPageHasFeature(si->_info, LevelProvider::QueryID)) {
        ...
        // v2.0.0: single-record levels used to return before any state or
        // action buttons were set up, leaving them unplayable from the popup.
        ...
        if (more.size() == 1) {
            ...
            popup->requestDateEstimation(prov);
...
void ProviderPopup::setupGenericSearchPage(CCLayer *providerBox) {
    ...
    in->setCallback([this](const std::string &value) {
        ...
        Mod::get()->setSavedValue<std::string>("last-query", value);
    });

    auto popup = ProviderPopup::get();

    // v2.0.0 fix: this checked _enterLevelID (the other page's field), so the
    // query was only restored if a level ID had also been typed.
    if (!popup->_enterQuery.empty()) {
...
void ProviderPopup::lambdaOnDownloadLevelList(SearchInstance *si, LoadingCircleLayer *existingCircle, ProviderPopup *popup, LevelProvider *prov, GJGameLevel *level) {
    if (!popup->isPageHasFeature(si->_info, LevelProvider::QueryLevelName)) {
        ...
        // v2.0.0: same fix as lambdaOnDownloadLevel — single search hits used
        // to skip state setup and action buttons.
        popup->_levelPage._currentLevels = more;
        popup->_levelPage._currentLevelsIndex = 0;
        popup->_levelPage.page = si->_page;
...
void ProviderPopup::onPlayLevelDownload(CCObject *sender) {
    ...
    popup->_selectedProvider->getLevelData(level->m_levelID.value(), [popup, level, wait] (LevelProvider *prov, std::string data, struct LevelProvider::BasicLevelInformation info) {
        ...
        if (data[0] == '-') {
            // v2.0.0: Cloudflare blocked the .gmd download — the API routes
            // are exempt but the website download route may be challenged.
            // Offer to open the level page in the browser instead.
            if (data == "-11") {
                auto clevel = newpopup->_levelPage._currentLevels[newpopup->_levelPage._currentLevelsIndex];
                int levelID = clevel->m_levelID.value();
                std::string url = prov->getWebsiteLevelURL(levelID);

                std::string err = fmt::format("<cr>Error while getting level data: </c><cy>{}</c>", prov->getErrorCodeDescription(data));

                if (!url.empty()) {
                    geode::createQuickPopup("Error", err, "OK", "Open Page", [url](FLAlertLayer *, bool btn2) {
                        if (btn2) {
                            geode::utils::web::openLinkInBrowser(url);
                        }
                    });
                } else {
                    FLAlertLayer::create("Error", err, "OK")->show();
                }
...
void ProviderPopup::applyBottomButtons(CCLayer *page) {
    ...
    // v2.0.0: four actions now share the row, so buttons are smaller.
    float btnScale = 0.5f;

    if (platform != TargetPlatform::kTargetAndroid) {
        {
            auto spr4 = ButtonSprite::create("Copy ID");
            spr4->setScale(btnScale);
    ...
    // v2.0.0: archive the shown record as a .gmd file.
    {
        auto spr4 = ButtonSprite::create("Save .gmd");
        spr4->setScale(btnScale);
        spr4->setID("export-gmd-spr");

        auto btn4 = CCMenuItemSpriteExtra::create(
            spr4, this, menu_selector(ProviderPopup::onExportGmd)
        );
        btn4->setID("export-gmd-btn");

        levelActionsMenu->addChild(btn4);
        levelActionsMenu->updateLayout();
    }

    // v2.0.0: open the level's page on the provider website.
    {
        auto spr4 = ButtonSprite::create("History Page");
        spr4->setScale(btnScale);
        spr4->setID("history-page-spr");

        auto btn4 = CCMenuItemSpriteExtra::create(
            spr4, this, menu_selector(ProviderPopup::onOpenHistoryPage)
        );
        btn4->setID("history-page-btn");
...
// v2.0.0: format big numbers with thousands separators ("42,320,715").
...
void ProviderPopup::onProviderInfoBtn(CCObject *sender) {
    ...
    // v2.0.0: enrich the info popup with live archive statistics when the
    // provider supports them.
    if (popup->_selectedProvider->hasArchiveStats()) {
        Notification::create("Fetching archive stats...", NotificationIcon::Loading, 0.5f)->show();

        popup->_selectedProvider->getArchiveStats([name, description](LevelProvider *, struct LevelProvider::ArchiveStats stats) {
            ...
            if (stats.ok) {
                content += fmt::format(
                    "\n\n<cy>Archive stats:</c>\n"
                    "<cg>{}</c> levels tracked\n"
                    "<cg>{}</c> level strings saved\n"
                    "<cg>{}</c> songs\n"
                    "<cg>{}</c> users\n"
                    "<cg>{}</c> save files",
                    thousands(stats.levelCount),
                    thousands(stats.levelStringCount),
                    thousands(stats.songCount),
                    thousands(stats.userCount),
                    thousands(stats.saveCount)
                );
            }

            FLAlertLayer::create(name.c_str(), content, "OK")->show();
        });
...
void ProviderPopup::onOpenHistoryPage(CCObject *sender) {
    auto popup = ProviderPopup::get();

    if (popup->_levelPage._currentLevels.empty()) return;
    ...
    if (levelID <= 0) {
        Notification::create("Level has an invalid ID.", NotificationIcon::Error, 0.5f)->show();

        return;
    }

    std::string url = popup->_selectedProvider->getWebsiteLevelURL(levelID);

    if (url.empty()) {
        Notification::create("This provider has no website page.", NotificationIcon::Error, 0.5f)->show();
...
void ProviderPopup::onExportGmd(CCObject *sender) {
    auto popup = ProviderPopup::get();
    ...
    int levelID = level->m_levelID.value();
    int recordID = level->m_demonVotes;
    std::string levelName = level->m_levelName;

    popup->_selectedProvider->getLevelData(levelID, [popup, wait, levelID, recordID, levelName](LevelProvider *prov, std::string data, struct LevelProvider::BasicLevelInformation info) {
        auto newpopup = ProviderPopup::get();
        ...
        // getLevelData() always writes the raw server response to
        // <save dir>/temp.gmd before parsing it (including on cache hits),
        // so archiving is just a file copy — no extra request needed.
        auto saveDir = Mod::get()->getSaveDir();
        auto exportDir = saveDir / "exports";

        (void) geode::utils::file::createDirectoryAll(exportDir);
        ...
        auto target = exportDir / fmt::format("{}-{}-record{}.gmd", safeName, levelID, recordID);

        std::error_code ec;
        std::filesystem::copy_file(saveDir / "temp.gmd", target, std::filesystem::copy_options::overwrite_existing, ec);

        if (ec) {
            Notification::create("Failed to save .gmd file.", NotificationIcon::Error, 1.f)->show();

            return;
        }

        Notification::create(fmt::format("Saved {}", target.filename().string()), NotificationIcon::Success, 1.5f)->show();

        geode::utils::file::openFolder(exportDir);
    });
}

void ProviderPopup::requestDateEstimation(LevelProvider *prov) {
    if (!Mod::get()->getSettingValue<bool>("show-date-estimation")) return;
    if (!prov->hasDateEstimation()) return;

    // _cId is only set for level ID searches — estimating an upload date
    // only makes sense for a specific level ID.
    int cId = _levelPage._cId;

    if (cId <= 0) return;

    prov->getDateEstimation(cId, [cId](LevelProvider *, struct LevelProvider::DateEstimation est) {
        auto popup = ProviderPopup::get();

        if (!popup) return;
        if (popup->_levelPage._cId != cId) return;
        if (!est.ok) return;

        popup->showDateEstimation(est.approx, est.low, est.high);
    });
}

void ProviderPopup::showDateEstimation(std::string approx, std::string low, std::string high) {
    CCNode *page = getChildByIDRecursive("page");

    if (!page) return;

    CCNode *old = page->getChildByID("date-estimation");

    if (old) old->removeMeAndCleanup();

    std::string text;

    if (!approx.empty()) {
        text = fmt::format("Est. upload date: {}", approx);

        if (!low.empty() && !high.empty() && low != high) {
            text += fmt::format(" ({} - {})", low, high);
        }
    } else if (!low.empty() || !high.empty()) {
        text = fmt::format(
            "Est. upload date: between {} and {}",
            low.empty() ? "?" : low,
            high.empty() ? "?" : high
        );
    } else {
    ...
    CCLabelBMFont *l = CCLabelBMFont::create(text.c_str(), "chatFont.fnt");
    l->setScale(0.45f);
    l->setColor({255, 200, 80});
    l->setPosition(csz.width / 2, 50);
    l->setID("date-estimation");
