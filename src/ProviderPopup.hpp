...
public:
    ...
    // v2.0.0: archive a copy of the level as a .gmd file into the mod's
    // save directory (exports/).
    void onExportGmd(CCObject *sender);

    // v2.0.0: open the level's page on the provider website in the browser.
    void onOpenHistoryPage(CCObject *sender);

    // v2.0.0: display the estimated original upload date under the results.
    void showDateEstimation(std::string approx, std::string low, std::string high);

    // v2.0.0: fire the async date estimation request for the current search.
    void requestDateEstimation(LevelProvider *prov);
